...
<xsl:template name="html" xmlns:xhtml="http://www.w3.org/1999/xhtml" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xforms="http://www.w3.org/2002/xforms" xmlns:ev="http://www.w3.org/2001/xml-events" xmlns:xsd="http://www.w3.org/2001/XMLSchema" xmlns:msxsl="urn:schemas-microsoft-com:xslt" xmlns:exslt="http://exslt.org/common" xmlns:xalan="http://xml.apache.org/xalan">
	...
	<html>
		<xsl:copy-of select="@*"/>
		<xsl:comment>HTML elements and Javascript instructions generated by XSLTForms 1.4 (654) - Copyright (C) 2019 &lt;agenceXML&gt; - Alain COUTHURES - http://www.agencexml.com</xsl:comment>
		...
		<head>
			...
			<xsl:variable name="jsrevisionname"><xsl:if test="$script/config/properties/jsrevision = 'concat'">-1.4</xsl:if></xsl:variable>
			<xsl:variable name="jsrevisionparam"><xsl:if test="$script/config/properties/jsrevision != '' and $script/config/properties/jsrevision != ''">?<xsl:value-of select="$script/config/properties/jsrevision"/>=654</xsl:if></xsl:variable>
			<script type="text/javascript" id="xsltforms-src" src="{$resourcesdirjs}xsltforms{$jsrevisionname}.js{$jsrevisionparam}" data-uri="http://www.agencexml.com/xsltforms">
				<xsl:attribute name="data-version">654</xsl:attribute>
				<xsl:text>/* */</xsl:text>
			</script>
...
<xsl:template name="group" xmlns:svg="http://www.w3.org/2000/svg" xmlns:xhtml="http://www.w3.org/1999/xhtml" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xforms="http://www.w3.org/2002/xforms">
	...
	<xsl:choose>
		...
		<xsl:otherwise>
			...
			<xsl:element name="{$mainelt}" namespace="{$mainns}">
				<xsl:copy-of select="@*[local-name() != 'ref' and local-name() != 'id' and local-name() != 'class' and local-name() != 'navindex' and local-name() != 'appearance' and local-name() != 'collapse']"/>
				<xsl:call-template name="genid">
					<xsl:with-param name="workid" select="$workid"/>
				</xsl:call-template>
				<xsl:call-template name="style">
				...
				<xsl:choose>
					...
					<xsl:otherwise>
							...
							<xsl:if test="self::xforms:label">
								<xsl:apply-templates select=".">
									...
									<xsl:with-param name="appearance"><xsl:choose>
											<xsl:when test="contains($appearance, 'expanded') or ../@collapse = 'open'">groupExpanded</xsl:when>
											<xsl:when test="contains($appearance, 'collapsed') or ../@collapse = 'closed'">groupCollapsed</xsl:when>
						...
						<xsl:element name="{$itemelt}" namespace="{$itemns}">
							<xsl:attribute name="class">xforms-<xsl:value-of select="$type"/>-content<xsl:if test="contains($appearance, 'collapsed') or @collapse = 'closed'"> xforms-disabled</xsl:if></xsl:attribute>
							<xsl:apply-templates select="node()">
								<xsl:with-param name="parentworkid" select="$workid"/>
								<xsl:with-param name="appearance" select="'groupNone'"/>
							</xsl:apply-templates>
		...
		<xsl:otherwise>
			...
			<xsl:variable name="i">
				<xsl:choose>
					...
					<xsl:when test="$c = '@'">
						<xsl:variable name="t0"><xsl:call-template name="getName"><xsl:with-param name="s" select="substring($s2,2)"/></xsl:call-template></xsl:variable>
						<xsl:variable name="t"><xsl:value-of select="$t0"/><xsl:if test="contains($t0,':') and substring($s2,string-length($t0)+2,1) = '*'">*</xsl:if></xsl:variable>
						<xsl:choose>
							<xsl:when test="$t != ''">
...
<xsl:template match="xforms:select" mode="script" priority="1" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xforms="http://www.w3.org/2002/xforms">
	...
	<js xmlns="">
		...
		<xsl:call-template name="toScriptBinding"><xsl:with-param name="p" select="@ref"/></xsl:call-template>
		<xsl:text>,</xsl:text>
		<xsl:choose><xsl:when test="@selection='open'"><xsl:text>true</xsl:text></xsl:when><xsl:otherwise><xsl:text>false</xsl:text></xsl:otherwise></xsl:choose>
		<xsl:text>,</xsl:text>
