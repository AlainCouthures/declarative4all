...
var composeResponse = function(resp, components, callback) {
  ...
  if (fname.endsWith('.js') && !global.fs.existsSync(fname)) {
    if (!global.fs.existsSync(fname.substr(0, fname.length - 3) + '-BROKEN.js')) {
      fname = fname.substr(0, fname.length - 3) + '-INPR.js';
    } else {
      fname  = fname.substr(0, fname.length - 3) + '-BROKEN.js';
    }
  }
  //console.log(fname);
