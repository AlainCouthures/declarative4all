...
<dcterms:hasVersion>1.5.4 (660)</dcterms:hasVersion>
...
<xsl:template match="xhtml:html | html" xmlns:xhtml="http://www.w3.org/1999/xhtml" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xforms="http://www.w3.org/2002/xforms" xmlns:ev="http://www.w3.org/2001/xml-events" xmlns:xsd="http://www.w3.org/2001/XMLSchema" xmlns:msxsl="urn:schemas-microsoft-com:xslt" xmlns:exslt="http://exslt.org/common" xmlns:xalan="http://xml.apache.org/xalan">
	...
	<html>
		...
		<xsl:comment>HTML elements generated by XSLTForms 1.5.4 (660) - Copyright (C) 2021 &lt;agenceXML&gt; - Alain Couthures - http://www.agencexml.com</xsl:comment>
		...
		<head>
			...
			<xsl:variable name="jsrevisionname"><xsl:if test="$jsrevision = 'concat'">-1.5.4</xsl:if></xsl:variable>
			<xsl:variable name="jsrevisionparam"><xsl:if test="$jsrevision != '' and $jsrevision != 'concat'">?<xsl:value-of select="$jsrevision"/>=660</xsl:if></xsl:variable>
			<xsl:element name="{$xsltforms_replacement_for}script">
				...
				<xsl:attribute name="data-version">660</xsl:attribute>
				<xsl:text>/* */</xsl:text>
			</xsl:element>
		</head>
		<body>
