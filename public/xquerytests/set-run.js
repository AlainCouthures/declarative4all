...
var confeval = async function(t, resolve, reject) {
  ...
  if (t.env) {
    ...
    try {
      const jsret = Fleur.XQueryParser._xp2js(t.expression, [], [], 0);
      const jsresult = (new Fleur.Transpiler("ctx", "  ")).funcdef(jsret);
      const jsvalue = (await eval("(" + jsresult + ")(new Fleur.Context(null, null, [], []))")).item;
      resolve(jsvalue, t.name);
    } catch (e) {
      reject(e, t.name);
    }
...
function runtests() {
  ...
  getloop(function(res, testname) {
      ...
      testname: testname,
      result: Fleur.Serializer._serializeNodeToXQuery(res, false, "", false, null, null, '"')
    });
...
function run() {
  ...
  try {
    var jsret = Fleur.XQueryParser._xp2js(document.getElementById("expression").value, [], [], 0);
    var arr;
    arr = jsret;
  ...
  readytests(function(res) {
    ...
    document.getElementById("effectiveresult").innerHTML = snow + " - " + Fleur.Serializer._serializeNodeToXQuery(res, false, "", false, null, null, '"').replace(/&/mg, "&amp;").replace(/</mg, "&lt;");
  }, function(err) {
    var now = new Date();
    var snow = ((now.getHours() < 10)?"0":"") + now.getHours() +":"+ ((now.getMinutes() < 10)?"0":"") + now.getMinutes() +":"+ ((now.getSeconds() < 10)?"0":"") + now.getSeconds();
